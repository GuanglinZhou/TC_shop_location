#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Date    : 2017-11-05 17:53:56
# @Author  : jameszhou (xdzgl812@163.com)
# @Link    : ${link}
# @Version : $V1$
# Using Stacking method to merge the three model:xgb,rf,lgb;
# -*-coding:utf-8 -*-
import os
import math
import re
...
def getMallLoc():
    ...
    for ix, row in dfff.iterrows():
        MALL_LOC[row['mall_id']] = (
            row['mall_longitude'], row['mall_latitude'])
...
def train_predict_write(mallids):
    ...
    for mallid in mallids:
        ...
        del test['longi_lati']
        ################# 划分训练集验证集并训练 #################
        NumClass = getNumClass(mallid)
        print('类别个数', getNumClass(mallid))
        print("lightgbm...")
        Y = train.pop('ClassIndex')
        X = train
        stack_y = list(StratifiedKFold(Y, K_FOLDS))
        mall_data_train = np.zeros((X.shape[0], NUM_MODEL * NumClass))
        mall_data_test = np.zeros((test.shape[0], NUM_MODEL * NumClass))
        ...
        lgb_params = {
            ...
            'max_bin': 200,  # todo 500
            'metric': 'multi_error',
            'num_leaves': 50,
            'learning_rate': 0.03,  # todo 0.03
            'feature_fraction': 0.9,
            'bagging_fraction': 0.8,
            'bagging_freq': 5,
            'verbose': -1,
            'num_trees': 100,  # todo 200
            'num_class': NumClass
        }
        print('Start lgb training...')
        # 交叉验证得到1/K train_predict,重复K次得到完整的train_predict
        # train_lgb and valid_lgb are index list
        dataset_stack_test_lgb = np.zeros((test.shape[0], NumClass * len(stack_y)))
        for i_lgb, (train_lgb, valid_lgb) in enumerate(stack_y):
            print("Fold", i_lgb)
            X_train_lgb = np.array(X)[train_lgb]
            y_train_lgb = Y[train_lgb]
            X_valid_lgb = np.array(X)[valid_lgb]
            y_valid_lgb = Y[valid_lgb]
            lgb_train = lgb.Dataset(X_train_lgb, label=y_train_lgb)
            lgb_eval = lgb.Dataset(X_train_lgb, label=y_train_lgb)
            gbm = lgb.train(lgb_params,
                            lgb_train,
                            valid_sets=lgb_eval,
                            num_boost_round=20,
                            early_stopping_rounds=20)  # todo 20
            ################# Predict #################
            # 使用训练集中4/5的数据进行训练对剩下1/5数据predict
            lgb_valid_Y = gbm.predict(X_valid_lgb)
            # lgb_valid_Y_index = np.argmax(lgb_valid_Y.tolist())
            # print(lgb_valid_Y)
            mall_data_train[valid_lgb, :NumClass] = lgb_valid_Y
            # 对全部的test数据predict
            lgb_test_Y = gbm.predict(test)
            dataset_stack_test_lgb[:, i_lgb * NumClass:(i_lgb + 1) * NumClass] = lgb_test_Y
            # print(dataset_stack_test_lgb[:,i_lgb*NumClass:(i_lgb+1)*NumClass])
        dataset_stack_test_final = np.zeros((dataset_stack_test_lgb.shape[0], NumClass))
        for j_lgb in range(NumClass):
            data_temp = np.zeros((dataset_stack_test_lgb.shape[0],))
            # print(data_temp.shape)
            for k_lgb in range(len(stack_y)):
                # print(dataset_stack_test_lgb[:, k_lgb * NumClass].shape)
                data_temp += dataset_stack_test_lgb[:, k_lgb * NumClass]
            dataset_stack_test_final[:, j_lgb] = data_temp / len(stack_y)
        mall_data_test[:, 0:NumClass] = dataset_stack_test_final
        ...
        num_rounds = 100  # 迭代次数 todo 100
        #####################################
        dataset_stack_test_xgb = np.zeros((test.shape[0], NumClass * len(stack_y)))
        for i_xgb, (train_xgb, valid_xgb) in enumerate(stack_y):
            print("Fold", i_xgb)
            X_train_xgb = np.array(X)[train_xgb]
            y_train_xgb = Y[train_xgb]
            X_valid_xgb = np.array(X)[valid_xgb]
            y_valid_xgb = Y[valid_xgb]
            xgb_train = xgb.DMatrix(X_train_xgb, label=y_train_xgb)
            xgb_eval = xgb.DMatrix(X_train_xgb, label=y_train_xgb)
            # watchlist = [(xgb_train, 'train'), (xgb_eval, 'val')
            watchlist = [(xgb_train, 'train')]
            model_xgb = xgb.train(plst, xgb_train, num_rounds, watchlist, early_stopping_rounds=20)  # todo 20
            ################# Predict #################
            # 使用训练集中4/5的数据进行训练对剩下1/5数据predict
            xgb_valid_Y = model_xgb.predict(xgb.DMatrix(X_valid_xgb), ntree_limit=model_xgb.best_ntree_limit)
            # xgb_valid_Y_index = np.argmax(xgb_valid_Y.tolist())
            # print(xgb_valid_Y)
            mall_data_train[valid_xgb, NumClass:NumClass * 2] = xgb_valid_Y
            # 对全部的test数据predict
            xgb_test_Y = model_xgb.predict(xgb.DMatrix(test.values),
                                           ntree_limit=model_xgb.best_ntree_limit)  # 注意这里不加values会出错
            dataset_stack_test_xgb[:, i_xgb * NumClass:(i_xgb + 1) * NumClass] = xgb_test_Y
        dataset_stack_test_final = np.zeros((dataset_stack_test_xgb.shape[0], NumClass))
        for j_xgb in range(NumClass):
            data_temp = np.zeros((dataset_stack_test_xgb.shape[0],))
            # print(data_temp.shape)
            for k_xgb in range(len(stack_y)):
                data_temp += dataset_stack_test_xgb[:, k_xgb * NumClass]
            dataset_stack_test_final[:, j_xgb] = data_temp / len(stack_y)
        mall_data_test[:, NumClass:2 * NumClass] = dataset_stack_test_final

        ######################Random Forest####################################
        print('Start rf training...')
        model_rf = RandomForestClassifier(n_estimators=300, criterion='gini', max_depth=None,
        ...
        #####################################
        # 由于随机森林做多分类时是自己检测类别数的，当我做交叉验证，我的训练集可能不包含所有的类别，所以会出错。
        # 由此，将下面这段代码注释掉，改为对所有的训练数据训练模型，再对训练数据做预测，得到P3和T3，会出现过拟合，对结果造成影响
        # 但现在也不知道怎么处理RF了，先试验一下这个，看看效果吧。
        # dataset_stack_test_rf = np.zeros((test.shape[0], NumClass * len(stack_y)))
        # for i_rf, (train_rf, valid_rf) in enumerate(stack_y):
        #     print("Fold", i_rf)
        #     X_train_rf = np.array(X)[train_rf]
        #     y_train_rf = Y[train_rf]
        #     X_valid_rf = np.array(X)[valid_rf]
        #     y_valid_rf = Y[valid_rf]
        #     model_rf.fit(X_train_rf, y_train_rf)
        #     ################# Predict #################
        #     # 使用训练集中4/5的数据进行训练对剩下1/5数据predict
        #     rf_valid_Y = model_rf.predict_proba(X_valid_rf)
        #     mall_data_train[valid_rf, NumClass * 2:NumClass * 3] = rf_valid_Y
        #     # 对全部的test数据predict
        #     rf_test_Y = model_rf.predict_proba(test)
        #     dataset_stack_test_rf[:, i_rf * NumClass:(i_rf + 1) * NumClass] = rf_test_Y
        # dataset_stack_test_final = np.zeros((dataset_stack_test_rf.shape[0], NumClass))
        # for j_rf in range(NumClass):
        #     data_temp = np.zeros((dataset_stack_test_rf.shape[0],))
        #     for k_rf in range(len(stack_y)):
        #         data_temp += dataset_stack_test_rf[:, k_rf * NumClass]
        #     dataset_stack_test_final[:, j_rf] = data_temp / len(stack_y)
        model_rf.fit(X, Y)
        mall_data_train[:, NumClass * 2:NumClass * 3] = np.array(model_rf.predict_proba(X))
        dataset_stack_test_final = np.array(model_rf.predict_proba(test))
        mall_data_test[:, 2 * NumClass:3 * NumClass] = dataset_stack_test_final
        ######################(P1,P2,P3),(T1,T2,T3)已创建好，存放在mall_data_train，mall_data_test##############
        print("Start the second train and predict")
        second_train_X = pd.DataFrame(mall_data_train)
        second_train_Y = Y
        second_test = pd.DataFrame(mall_data_test)
        second_xgb_params = {
            'booster': 'gbtree',
        ...
        second_plst = list(second_xgb_params.items())
        num_rounds = 200  # todo 200 迭代次数
        second_xgtrain = xgb.DMatrix(second_train_X, label=second_train_Y)
        second_xgval = xgb.DMatrix(second_train_X, label=second_train_Y)
        second_watchlist = [(second_xgtrain, 'train')]
        second_model_xgb = xgb.train(second_plst, second_xgtrain, num_rounds, second_watchlist,
                                     early_stopping_rounds=20)
        ################# Predict #################
        second_test_Y = second_model_xgb.predict(xgb.DMatrix(second_test),
                                                 ntree_limit=second_model_xgb.best_ntree_limit)
        second_test_Y = pd.concat([test_rowId, pd.Series(second_test_Y)], axis=1)
        second_test_Y.columns = ['row_id', 'ClassIndex']
        shop_id = []
        for index, row in second_test_Y.iterrows():
            shop_id.append(classIndexShopMap[str(int(row['ClassIndex']))])
        second_test_Y['shop_id'] = pd.DataFrame(shop_id)
        del second_test_Y['ClassIndex']
        # 输出概率
        # probStrs = list()
        # for pyOne in second_test_Y:
        #     curStr = ','.join(map(str, pyOne))
        #     probStrs.append(curStr)
        # second_test_Y = pd.concat([test_rowId, pd.Series(probStrs)], axis=1)
        # second_test_Y.columns = ['row_id', 'class_prob']
        # 写入概率结果到文件
        second_test_Y.to_csv('Stack_Merge_Out_Cate/' + mallid + ".csv", index=False)

    # if TO_FILE == 1:
    #     with open('submit_' + dt.now().strftime("%m_%d_%H_%M") + "_Stack_Merge.csv", 'w') as f:
    #         f.write("row_id,shop_id\n")
    #         for mallid in mallids:
    #             df_each = pd.read_csv("Stack_Merge_Out_Cate/" + mallid + ".csv")
...
train_predict_write(['m_1293'])
# train_predict_write(['m_690'])
