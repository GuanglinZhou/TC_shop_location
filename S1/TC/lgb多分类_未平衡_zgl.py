...
lgb_train_final = lgb.Dataset(X, label=Y)
lgb_eval_final = lgb.Dataset(X, label=Y)
params = {
    'task': 'train',
    'boosting_type': 'gbdt',
...
################# 预测并构建结果 #################
print("Start testing...")
# lightgbm predict result is a ndarray,and
# For multi-class task, the y_pred is group
#  by class_id first, then group by row_id,so need a transfer
test_Y = gbm.predict(test)
max_index = np.argmax(test_Y, axis=1).tolist()
test_Y = pd.concat([test_rowId, pd.Series(max_index)], axis=1)
...
# 写入结果到文件
test_Y.to_csv(DIR_PREFIX + "result_lgb/lgb_" + mallid + ".csv", index=False)
...
data = pd.DataFrame(np.RandomForestClassifier)
