...
########################  train and predict  ##############################
#############################################################
def train_predict_write(mallids, lgb_coe, xgb_coe, rf_coe):
    global TO_FILE
    global DIR_PREFIX
    ...
    for mallid in mallids:
        ...
        lgb_eval = lgb.Dataset(X, label=Y)
        lgb_params = {
            'task': 'train',
            'boosting_type': 'gbdt',
        ...
        # 写入概率结果到文件
        test_Y.to_csv('model_output_prob/' + mallid + "_model1.csv", index=False)
        ...
        xgval = xgb.DMatrix(X, label=Y)
        watchlist = [(xgtrain, 'train'), (xgval, 'val')]
        model = xgb.train(plst, xgtrain, num_rounds, watchlist, early_stopping_rounds=20)
        ################# Predict #################
        xgb_test_Y = model.predict(xgb.DMatrix(test), ntree_limit=model.best_ntree_limit)
        test_Y = xgb_test_Y
        xgb_test_Y = xgb_test_Y *  xgb_coe
        # 输出概率
        probStrs = list()
        for pyOne in test_Y:
            curStr = ','.join(map(str, pyOne))
        ...
        ################# Predict #################
        # 输出概率
        rf_test_Y = model.predict_proba(test)
        test_Y = rf_test_Y
        rf_test_Y = rf_test_Y * rf_coe
        probStrs = list()
        for pyOne in test_Y:
            curStr = ','.join(map(str, pyOne))
            probStrs.append(curStr)
        ...
        test_Y.columns = ['row_id', 'class_prob']
        # 写入概率结果到文件
        test_Y.to_csv('model_output_prob/' + mallid + "_model3.csv", index=False)
        ############################# Merge ###################################

        lgb_max_index = np.argmax(lgb_test_Y, axis=1).tolist()
        lgb_max_value = np.max(lgb_test_Y, axis=1).tolist()
        xgb_max_index = np.argmax(xgb_test_Y, axis=1).tolist()
        xgb_max_value = np.max(xgb_test_Y, axis=1).tolist()
        rf_max_index = np.argmax(rf_test_Y, axis=1).tolist()
        rf_max_value = np.max(rf_test_Y, axis=1).tolist()
        result_y = []
        for i in range(test.shape[0]):
            max_value = max(lgb_max_value[i], xgb_max_value[i], rf_max_value[i])

            if (lgb_max_value[i] == max_value):
                result_y.append(lgb_max_index[i])
            elif (xgb_max_value[i] == max_value):
                result_y.append(xgb_max_index[i])
            else:
                result_y.append(rf_max_index[i])

        result_y = pd.concat([test_rowId, pd.Series(result_y)], axis=1)
        ...
        # 写入结果到文件
        result_y.to_csv(DIR_PREFIX + "result_lgb_xgb_rf/" + mallid + "_lgb_xgb_rf.csv", index=False)
        ####### 只用于 所有mall 预测   #######
    if TO_FILE == 1:
        with open('submit_' + dt.now().strftime("%m_%d_%H_%M") + ".csv", 'w') as f:
            f.write("row_id,shop_id\n")
            # for mallid in getMalls():
            df_each = pd.read_csv(DIR_PREFIX + "result_lgb_xgb_rf/" + mallid + "_lgb_xgb_rf.csv")
...
TO_FILE=1
###################################################################
train_predict_write(getMalls(), 0.3, 0.4, 0.3)
